...
version: 1.1.0
argument-hint: "[agent-name] [version]"
...
1. Resolve the agent name from $ARGUMENTS (first token).
   If missing, scan all `.claude/skills/*/PROPOSAL-*.md` files for those with `status: pending`,
   list them, and ask the user which agent to update.

2. If a version is provided as the second token, read `.claude/skills/{agent-name}/PROPOSAL-v{version}.md`.
   Otherwise, list all `PROPOSAL-*.md` files in the agent's skill folder whose frontmatter has `status: pending`.
...
3. Read the full proposal. Extract: agent, current-version, proposed-version, change-type, breaking (yes/no), affected-agents, and all proposed changes.

### Validate

4. Read `.claude/skills/{agent-name}/SKILL.md`. Check that the `version` field in its frontmatter matches the proposal's `current-version`.
...
4b. If `change-type` is `major`:
   ...
   b. Ask: "This is a major structural change. Would you like to validate the new design with /anthropic-architect before applying? (y/n)"
   c. If yes: pause and instruct the user to run `/anthropic-architect` with the proposed changes as input, then return here once validated.
...
5. For each "Proposed Skill Changes" section in the proposal:
   ...
   c. If the exact text cannot be found, show both texts and the diff, then ask:
      "Could not find the exact current text in {agent-name}/SKILL.md. Apply manually? Paste the section to replace, or skip this change."
...
7. If the proposal has `Breaking: yes` and lists affected agents:
   a. For each affected agent, read their SKILL.md and identify what needs to change due to the format or interface change.
   b. Describe the required change clearly: "Agent {name} calls {target agent} and expects {old format}. It needs to be updated to expect {new format}."
   c. Ask: "Agent {name} needs a cascade update. Apply it automatically? (y/n)"
   d. If yes:
...
9. If `.claude/skills/{agent-name}/CHANGELOG.md` does not exist, create it with:
   ```
...
11. Print:
    ...
    • {affected-agent}: {old} → {new} (patch) — CHANGELOG updated

    {If cascade updates were declined:}
    Manual updates needed:
    • {affected-agent}: {description of what needs to change}

    Run /review-agents {slug} after the next session to continue improving agents.
    ════════════════════════════════════════════════════
