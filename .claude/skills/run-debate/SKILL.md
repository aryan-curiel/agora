...
4. Read .claude/skills/lead-agent/SKILL.md to understand how to invoke the lead agent.

### Invoke the lead agent

5. Invoke /lead-agent with the idea slug as argument.
...
Default: 4 rounds. Check CLAUDE.md for configured limits.
...
7. For each specialist in the roster:
   a. Read .claude/skills/{agent-name}/MEMORY.md — if the file exists, load its content as [YOUR MEMORY].
      If the file does not exist, omit [YOUR MEMORY] from the context.
   b. Invoke the specialist skill as /agent-{name} with context containing:
      ...
      - Messages from earlier this round (so each agent sees what others said)
...
8. After all specialists in the round have responded, invoke /score-round with:
   - The idea slug
   - All messages from this round
   - Current scores from the idea file

9. /score-round returns updated scores and synthesis. Print a milestone update:
...
12. Invoke /write-report with the session data.
...
15. For each specialist in the roster, trigger a memory update:
    a. Read .claude/skills/{agent-name}/MEMORY.md — load its content as [CURRENT MEMORY].
       If the file does not exist, use empty string.
    b. Collect all messages from this agent across all rounds as [YOUR CONTRIBUTIONS],
       labeled "Round {n}: {message content}".
    c. Use the synthesis from the final score-round output as [SESSION SYNTHESIS].
    d. Invoke /agent-{name} with:
       - MODE: memory-update
       - [CURRENT MEMORY]: {content from step a}
       - [YOUR CONTRIBUTIONS]: {content from step b}
       - [SESSION SYNTHESIS]: {content from step c}
       - [DATE]: {today's date YYYY-MM-DD}
    e. The agent returns ONLY the updated MEMORY.md content (a markdown document).
    f. Write the returned content to .claude/skills/{agent-name}/MEMORY.md.
...
16. Print final summary:
    ...
    Agent memories updated: {comma-separated list of agents whose MEMORY.md was written}

    Run /run-debate {slug} to continue developing this idea.
    Run /review-agents {slug} to review agent performance and generate improvement proposals.
